    ...
    dependencies = {
        {
            ...
            config = function()
                ...
                -- select
                vim.keymap.set("n", "<leader>sc", function() tlcBuiltin.colorscheme(require("telescope.themes").get_dropdown({ enable_preview = true })) end, { desc = "select [c]olorbtheme" })
                vim.keymap.set("n", "<leader>sb", function() tlcBuiltin.buffers(require("telescope.themes").get_dropdown({ enable_preview = false })) end, { desc = "select [b]uffer" })
                -- # insert mode
